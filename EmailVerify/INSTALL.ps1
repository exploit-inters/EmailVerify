param (
    [string]$sname = "EmailVerify",
    [string]$sdname = "Email verify",
    [string]$sdesc = "Служба верификации email",
    [string]$spath = "C:\TravelServices\EmailVerify\"
)
...
#Content
$exe = "EmailVerify.exe"
$cfg = "EmailVerify.exe.config"
$dlls = Get-ChildItem $PSScriptRoot -Filter "*.dll"

Write-Host "***TravelConfigurator installation***"
...
if ($service){
    ...
    $upgrade = $true
    $servicePath = (Get-WmiObject win32_service | ?{$_.Name -eq $sname}).PathName -replace "EmailVerify.exe", "" -replace """", ""
