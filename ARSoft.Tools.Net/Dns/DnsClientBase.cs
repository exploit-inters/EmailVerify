			...
			where TMessage : DnsMessageBase, new()
		{
			...
			for (var i = 0; i < endpointInfos.Count; i++)
			{
				...
				try
				{
					...
					var resultData = sendByTcp ? QueryByTcp(endpointInfo.ServerAddress, messageData, messageLength, ref tcpClient, ref tcpStream, out responderAddress) : QueryByUdp(endpointInfo, messageData, messageLength, out responderAddress);
			...
			if (IsResponseValidationEnabled)
			{
				if ((result.ReturnCode == ReturnCode.NoError) || (result.ReturnCode == ReturnCode.NxDomain))
				{
					...
					for (var j = 0; j < message.Questions.Count; j++)
					{
						...
						var responseQuestion = message.Questions[j];
